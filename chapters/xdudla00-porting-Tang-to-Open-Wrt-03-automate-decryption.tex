...
People usually try to automate everything they can in order to avoid repetitive tasks. % miso
...
As shown in table \ref{compare} Comparing Escrow and Tang, Tang compared to Escrow is stateless and doesn't require TLS or authentication. % miso
...
Since, these packets are blinded by eJWK, only the party that can unblind these values is the client itself (since only it has eJWK's private key). % miso
...
Keys are stored in the Tang's database directory and update script, which is activated by systemd, generates the cache in a cache directory. % miso
...
Secondly, {\it CONFIG} is a JSON object which will be passed directly to the {\it PIN}. %miso
