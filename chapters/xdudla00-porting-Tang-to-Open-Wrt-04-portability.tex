...
This platform has come a long way in making itself more accessible to all user levels.

In the past...

DD features
Tomato gui  Smaller router compatibility list
OpenWrt package
...
It may be more stable than our hardware’s default firmware from vendor.
Not even that but probably more secure with regullar updates and with support of key-based SSH.

It is capable of running lightweight services like an IRC bouncer or samba/ftp file sharing(some routers has USB ports able to power HDD).
...
With Tang we do not have to care about typing passphrases over and over to unlock LUKS drives in safe enviroment \cite{vpnpick}.
...
The another reasons, not to just compare internal storage which might be extended (as shown on apendix TODOapendix), are device's minimalistic RAM size and lower computation capability of the CPU.
