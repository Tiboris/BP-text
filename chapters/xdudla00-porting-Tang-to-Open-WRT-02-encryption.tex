...
More secure way to create passwords would be to generate cryptographically stronger cypher an use it as password.
The only disadvantage is that it is hard to remember \cite{splashdata}.

\section{Hard drive encryption} \todo{rework}
...
\begin{figure}[h]
    ...
    \label{fig:encdata}
\end{figure}
However, this secret data might grow in size, and it is time and resource consuming to decrypt and encrypt all our data every time the encryption key changes or it is compromised.
...
It can be changed whenever user desire to, and redistributed to all users or services who are supposed to access this data.


that could disrupt our daily basis.
...
Same as with the passwords ... more secure way to create key encryption key would be to generate cryptographically stronger key encryption key than password provided by user.
