...
\\\\
The exhibit shows that on our "host" system we have installed an NVMe (NVM Express) volume\footnote{https://en.wikipedia.org/wiki/NVM\_Express} using msdos, therefore MBR partition table.
...
\\\\
\texttt{\# cryptsetup isLuks /dev/nvme0n1p1 -v\\
...
\\\\
\texttt{\# cryptsetup isLuks /dev/nvme0n1p2 -v\\
Command successful.\\
}
\\\\
...
\\\\
\texttt{\# cryptsetup luksDump /dev/nvme0n1p2\\
...
\\\\
From the listing we can assume that /dev/nvme0n1p2 has one key encryption key in slot 0 and has other 7 slots disabled.
Add key:
  sudo cryptsetup luksAddKey  /dev/nvme0n1p2
[sudo] password for tdudlak:
...
sudo cryptsetup luksKillSlot  /dev/nvme0n1p2 3
