...
Companies define their own security strategies which may include encryption or may not at all. % miso
Security strategies rely on company's needs or will to take risks in conclusion of getting high gain from them. % miso
...
To create strong password we can follow any trustworthy guide\footnote{https://www.wikihow.com/Create-a-Secure-Password} on the Internet. % https://www.centos.org/docs/4/html/rhel-sg-en-4/s1-wstation-pass.html\#S2-WSTATION-PASS-CREATE
...
Encrypting MBR would make it impossible to start boot sequence of operating system. % miso
Boot sequence would prompt the user for key encryption key in order to load the operating system from encrypted storage. % miso
...
It works with BitLocker to help protect user data and to ensure that a computer has not been tampered with while the system was offline\cite{bitlocker}. % miso
...
The advantages of LUKS over plain dm-crypt are the higher usability: automatic configuration of non-default crypto parameters, the ability to add, change, and remove multiple passphrases. % miso preco dvojbodka?
...
Lets sum up the easier way first. % miso
...
For this purpose, the LUKS In-Place Conversion Tool {\it luksipc} was developed. % miso
Steps to encrypt a disk using {\it luksipc} are in the appendix \ref{luksipc} LUKS In-Place Encryption. % miso



\subsection{LUKS drive structure}

A hard drive with a LUKS partition has notable structure see Figure \ref{fig_luksvol} LUKS volume structure for a demonstration. % miso
...
The passphrases stored in the key slots, which structure is shown on Figure \ref{fig_luksslot} LUKS Key Slot, are used to decrypt a single key encryption key that is stored in the anti-forensic stripes.
...
This is a demonstration of how managing LUKS volume could look like in reality, in our case on our "host" Fedora 27 system. % miso
...
The actual result of the command is returned as an exit code. % miso clen
The output shown reflects that our boot partition is not a valid LUKS volume as expected. % miso
